...
class DriveManager:
    ...
    def upload_file(self, local_path, region_tag):
        ...
        existing = self.drive.ListFile(
            {'q': f"title='{final_name}' and '{self.folder_id}' in parents and trashed=false"}).GetList()
        if existing:
            print(f"[CLOUD] ✅ {final_name} exists (Skipping)")
            return existing[0]['id']

        print(f"[CLOUD] ⏳ Uploading {final_name}...")
        gfile = self.drive.CreateFile({'title': final_name, 'parents': [{'id': self.folder_id}]})
...
class HybridDownloader:
        ...
        if not (os.path.exists(zip_path) and zipfile.is_zipfile(zip_path)):
            ...
            while not downloaded:
                try:
                    ...
                    m = 'wb'
                    if curr > 0: headers['Range'] = f"bytes={curr}-"; m = 'ab'

                    with requests.get(final_url, headers=headers, stream=True, timeout=60) as r:
        ...
        try:
            with zipfile.ZipFile(zip_path, 'r') as z:
                for info in z.infolist():
                    target = None
                    if mode == "S2":
                        if ("_B04_" in info.filename or "_B08_" in info.filename) and info.filename.endswith(
                                ".jp2") and "R60m" not in info.filename:
                            suffix = "RED.jp2" if "_B04_" in info.filename else "NIR.jp2"
                            target = os.path.join(temp_dir, f"{prod_name}_{suffix}")
                    elif mode == "S1":
                        if "measurement" in info.filename and "-vv-" in info.filename.lower() and info.filename.endswith(
                                ".tiff"):
                            target = os.path.join(temp_dir, f"{prod_name}_VV.tiff")

                    if target:
                        with open(target, "wb") as f: f.write(z.read(info.filename))
                        files.append(target)
        except Exception as e:
    ...
    def process_region(self, region_name, bbox, temp_dir):
        ...
        polygon = (
            f"SRID=4326;POLYGON(({bbox[0]} {bbox[1]}, {bbox[2]} {bbox[1]}, {bbox[2]} {bbox[3]}, {bbox[0]} {bbox[3]}, {bbox[0]} {bbox[1]}))")

        # Try S2 (Clear)
        print(f"\n--- Processing Region: {region_name} ---")
        filter_s2 = (
            f"Collection/Name eq 'SENTINEL-2' and OData.CSC.Intersects(area=geography'{polygon}') and ContentDate/Start ge 2024-01-01T00:00:00.000Z and Attributes/OData.CSC.DoubleAttribute/any(att:att/Name eq 'cloudCover' and att/Value lt 20.00)")
        r = requests.get(self.SEARCH_URL,
                         params={"$filter": filter_s2, "$orderby": "ContentDate/Start desc", "$top": 1},
                         headers={"Authorization": f"Bearer {self.token}"})
        s2_res = r.json().get('value', [])

        if s2_res: return self._download_and_extract(s2_res[0], temp_dir, "S2")

        # Try S1 (Radar)
        print(f"[CDSE] {region_name} is cloudy. Switching to Radar.")
        filter_s1 = (
            f"Collection/Name eq 'SENTINEL-1' and OData.CSC.Intersects(area=geography'{polygon}') and ContentDate/Start ge 2024-01-01T00:00:00.000Z and Attributes/OData.CSC.StringAttribute/any(att:att/Name eq 'productType' and att/Value eq 'GRD') and Attributes/OData.CSC.StringAttribute/any(att:att/Name eq 'sensorMode' and att/Value eq 'IW')")
        r = requests.get(self.SEARCH_URL,
                         params={"$filter": filter_s1, "$orderby": "ContentDate/Start desc", "$top": 1},
                         headers={"Authorization": f"Bearer {self.token}"})
        s1_res = r.json().get('value', [])

        if s1_res: return self._download_and_extract(s1_res[0], temp_dir, "S1")
        return []
