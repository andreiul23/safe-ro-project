...
def get_gee_client():
    try:
        ...
        st.error(
            "Missing GEE project configuration. Please create a file `.streamlit/secrets.toml` and add `gee_project = 'your-project-id'`."
        )
        st.stop()
...
mode = st.sidebar.radio(
    "Mode:", ["Home", "Citizen App", "Authority Dashboard", "Local Analysis"]
)
...
# --- MODE: HOME ---
if mode == "Home":
    st.title("Welcome to SAFE-RO")
    col1, col2 = st.columns([3, 2])
    with col1:
        st.markdown("### 🌍 Real-time Disaster Monitoring with Google Earth Engine")
        st.write(
            "SAFE-RO uses **Google Earth Engine** to analyze **Sentinel-1 (Radar)** and **Sentinel-2 (Optical)** data for flood detection and vegetation monitoring across Romania."
        )
        st.image(
...
# --- MODE: CITIZEN APP ---
    ...
    st.markdown(
        '<div class="safe-box">✅ STATUS: Monitoring services are active.</div>',
        unsafe_allow_html=True,
    )
...
# --- MODE: LOCAL ANALYSIS ---
    ...
    tab1, tab2, tab3 = st.tabs(
        ["☁️ Cloud Data (Google Drive)", "📂 Manual Upload", "🔥 Fire Monitor"]
    )
    ...
    with tab1:
        ...
        if gdrive_client.drive:
            ...
            if not files:
                st.warning(
                    "Connected, but 'SAFE_RO_Cloud_Data' folder is empty or missing."
                )
            else:
                ...
                if analysis_type == "NDVI (Vegetation)":
                    ...
                    with col1:
                        red_choice = st.selectbox(
                            "Select RED Band", list(red_map.keys())
                        )
                    with col2:
                        nir_choice = st.selectbox(
                            "Select NIR Band", list(nir_map.keys())
                        )

                    if st.button("🚀 Analyze NDVI Cloud Data"):
                        if red_choice and nir_choice:
                            with st.spinner("Downloading from Google Drive..."):
                                path_red = gdrive_client.download_file(
                                    red_map[red_choice]
                                )
                                path_nir = gdrive_client.download_file(
                                    nir_map[nir_choice]
                                )
                ...
                # Flood Analysis from Cloud
                    ...
                    radar_choice = st.selectbox(
                        "Select Radar Image", list(radar_map.keys())
                    )
                    if st.button("🚀 Analyze Flood Cloud Data"):
                        if radar_choice:
                            with st.spinner("Downloading from Google Drive..."):
                                path_radar = gdrive_client.download_file(
                                    radar_map[radar_choice]
                                )
                            if path_radar:
                                try:
        ...
        if analysis_type == "NDVI (Vegetation)":
            ...
            if st.button("🚀 Analyze Local NDVI Files"):
                if uploaded_red and uploaded_nir:
                    ...
                    ) as tr, tempfile.NamedTemporaryFile(
                        delete=False, suffix=".tif"
                    ) as tn:
            ...
            uploaded_radar = st.file_uploader(
                "Upload Sentinel-1 VV Band (.tif)", key="m_radar"
            )
            ...
            if st.button("🛰️ Fetch Active Fire Data"):
                with st.spinner("Querying FIRMS..."):
                    try:
                        ...
                        fire_data = firms_client.get_active_fires(
                            current_bbox, end_date
                        )
